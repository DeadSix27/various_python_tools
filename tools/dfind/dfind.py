...
# into an SQlite Database file and uses that to very quick searches.
#
# ################################################
#
# Simple usage:
...
# the same directoy as this script, so make sure its writeable
#
# Default: dfind
#
INDEX_PREFIX = 'dfind'
...
def getDriveRoots(ignoredRoots = (), customPlaces = ()):
	drives = win32api.GetLogicalDriveStrings()
	drives = drives.split('\000')[:-1]
	drives = [x for x in drives if not x.lower().startswith(tuple(x.lower() for x in ignoredRoots))]
...
def indexDrives(singleThreaded=False):
	...
	if not singleThreaded:
		thrList = []
		for i, d in enumerate(getDriveRoots(IGNORED_DRIVES, CUSTOM_PLACES)):
		...
		for d in getDriveRoots(IGNORED_DRIVES, CUSTOM_PLACES):
...
if __name__ == '__main__':
	...
	parser.add_argument('-e', '--exact-match', help='Do not use wildcard search (default: yes)', dest='noWildCard', action='store_true', default=False)
	parser.add_argument('-c', '--case-sensitive', help='Search case-sensitively (default: no)', dest='caseSensitive', action='store_true', default=False)
	parser.add_argument('-u', '--with-ui', help='Show UI with search results (default: yes)', dest='withUi', action='store_true', default=False)
	...
	if args.withUi:
		...
		results = find(args.search, args.noWildCard, args.caseSensitive).List
		if len(results):
			for x in results:
				print(x.FullPath)
		else:
